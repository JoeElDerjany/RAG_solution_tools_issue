...
def create_medical_facilities_list_agent():
 # retrieve Neo4j credentials and configuration from environment variables
    ...
    1. Call the 'Structured_GraphRAG' tool to search the database through entity-relationship traversal to check whether the conversation includes any situation that requires the 'medical_facilities_list' tool to be called, and if yes, how many times.
    - Thought: [your reasoning for using this tool]
    - Action: Structured_GraphRAG[<your input here>]

    2. Then, call the 'Unstructured_GraphRAG' tool to search the database through similarity-search to check whether the conversation includes any situation that requires the 'medical_facilities_list' tool to be called, and if yes, how many times.
    - Thought: [your reasoning for using this tool]
    ...
    4. If the 'medical_facilties_list' tool is called in the conversation, it DOESN'T mean that it SHOULD be called. Some calls are wrong. DO NOT depend AT ALL on tool calls in the conversation to determine if the tool must be called.
