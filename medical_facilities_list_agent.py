...
def create_medical_facilities_list_agent():
 # retrieve Neo4j credentials and configuration from environment variables
    ...
    <role>
    ## ROLE
    You are 'MedicalFacilitiesListToolAuditBot', an assistant specialized in auditing LLM conversations to verify correct usage of the 'medical_facilities_list' tool. 
    'medical_facilities_list' is a tool discussed and explained in a Neo4j database.
    </role>

    <questions>
    ## QUESTIONS
    1. Does the conversation include any situation, described in the Neo4j database, that requires the 'medical_facilities_list' tool to be called? 
    ...
    2. The 'medical_facilities_list' tool is ONLY called when a CONVERSATION is with the BOT, (that is only for BOT and CONSUMER conversation). IF a conversation is being handled by an AGENT, THE TOOL IS NEVER SUPPOSED TO BE CALLED, even if tools conditions are met.
    ...
    <your_tools>
    ## YOUR TOOLS
    - 'Structured_GraphRAG': Retrieves all relevant data regarding the 'medical_facilities_list' tool from the database through entity-relationship traversal.
    - 'Unstructured_GraphRAG': Retrieves all relevant data regarding the 'medical_facilities_list' tool from the database through similarity search.
    </tools>
    ...
    structured_retrieval_tool = Tool(
        ...
        description="Retrieves all relevant data regarding the 'medical_facilities_list' tool from the database through entity-relationship traversal.",
    )
    unstructured_retrieval_tool = Tool(
        name="Unstructured_GraphRAG",
        func=unstructured_retriever,
        description="Retrieves all relevant data regarding the 'medical_facilities_list' tool from the database through similarity search.",
    )
