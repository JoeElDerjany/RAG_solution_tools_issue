...
def create_send_document_agent():
 # retrieve Neo4j credentials and configuration from environment variables
    ...
    1. Call the 'Structured_GraphRAG' tool to search the database through entity-relationship traversal to check whether the conversation includes any situation that requires the 'send_document' tool to be called, and if yes, how many times.
    - Thought: [your reasoning for using this tool]
    - Action: Structured_GraphRAG[<your input here>]

    2. Then, call the 'Unstructured_GraphRAG' tool to search the database through similarity-search to check whether the conversation includes any situation that requires the 'send_document' tool to be called, and if yes, how many times.
    - Thought: [your reasoning for using this tool]
    ...
    3. The 'send_document' tool is ONLY called when a CONVERSATION is with the BOT, (that is for BOT and CONSUMER conversation). IF a conversation is being handled by an AGENT without ANY BOT messages, THE TOOL IS NEVER SUPPOSED TO BE CALLED, even if tools conditions are met. However, if both an agent and a bot handled the conversation, we must check the conditions.
