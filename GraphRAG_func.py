...
def createAgent():
 # retrieve Neo4j credentials and configuration from environment variables
    ...
    **A tool is ONLY supposed to be called when the conversation is with the BOT.**
    ...
    <task>
    ## TASK
    Always adhere to the following, to verify the tool usage:
    1. 1. First, apply the '<guiding_principle>' to the entire conversation and then carefully read the entire conversation JSON provided by the user.
    2. Use the list of '<supported_tools>' (provided below) as the complete set of tools that can be called in a conversation.
    3. For each supported tool, determine from the Neo4j database:
    a. Whether the conversation context implies it SHOULD have been called.
    b. How many times it SHOULD have been called.
    4. To determine the required in step 3:
    a. Use the 'Structured_GraphRAG' tool to search the database through entity-relationship traversal.
    b. Then, use the 'Unstructured_GraphRAG' tool to search the database through similarity-search.
    5. Do step 3 for every tool at a time. Be done with 'transfer_chat' before starting with 'send_document', then 'medical_facilities_list' then 'open_a_complaint' then 'insurance_covered'.
    CRITICAL NOTE: A tool is ONLY called when a CONVERSATION is with the BOT, (that is only for BOT and CONSUMER conversation). IF a conversation is being handled by an AGENT, a TOOL IS NEVER SUPPOSED TO BE CALLED, even if tools conditions are met.
    6. Output your final answer as a single JSON object (no extra text), matching exactly the 'Output Schema' below.
    7. When making 'SHOULD have been called' decisions, think step-by-step:
    ...
    NOTE: For 'numberTimes_Supposed_To_Be_Called': If a request or trigger appears multiple times (even if repeated in adjacent messages), consider each as a SEPARATE and independent reason to call the tool AND increase the count for 'numberTimes_Supposed_To_Be_Called'
    </task>
