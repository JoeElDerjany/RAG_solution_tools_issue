...
def create_insurance_covered_agent():
 # retrieve Neo4j credentials and configuration from environment variables
    ...
    1. Call the 'Structured_GraphRAG' tool to search the database through entity-relationship traversal to check whether the conversation includes any situation that requires the 'insurance_covered' tool to be called, and if yes, how many times.
    - Thought: [your reasoning for using this tool]
    - Action: Structured_GraphRAG[<your input here>]

    2. Then, call the 'Unstructured_GraphRAG' tool to search the database through similarity-search to check whether the conversation includes any situation that requires the 'insurance_covered' tool to be called, and if yes, how many times.
    - Thought: [your reasoning for using this tool]
